    ...
    public String toString() {
        ...
        if(streamdata.length > 0) {
            for(int i = 0; i < streamdata.length; i++) {
                if((i % 16) == 0) {
                    sb.append(JDeserializer.linesep).append(String.format("%7x: ", Integer.valueOf(i)));
                }
                sb.append(" ").append(JDeserializer.hexnoprefix(streamdata[i]));
            }
            sb.append(JDeserializer.linesep);
        }
        sb.append("]");
        return sb.toString();
    }
